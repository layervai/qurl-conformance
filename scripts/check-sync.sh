...
cd "$(dirname "$0")/.."
fail=0
for f in qv2_conformance_vectors.json issuer_signature_vectors.json relay_knock_golden.json agent_registration_golden.json agent_assignment_golden.json agent_knock_application_vectors.json agent_session_control_vectors.json agent_api_key_id_vectors.json assignment_ticket_v1_vectors.json connector_authority_lambda_v1_vectors.json connector_hub_request_id_v1_vectors.json connector_hub_lst_cookie_v1_vectors.json agent_credential_recovery_v1_vectors.json crid_v1_vectors.json; do
